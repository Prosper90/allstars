...
const Modal = ({ currentDetails, setOpenModal, textureSelected }) => {
  ...
  return (
    <div className='bg-[#00000069] fixed h-screen overflow-y-scroll px-5 w-full top-0 z-30 flex flex-col ' >
      <div className="absolute text-[#fff] top-5 right-10">
        <button onClick={() => setOpenModal(!true)}>close</button>
      </div>
      <div className='absolute top-[50%] left-[50%] md:border  mt-8 md:mt-auto  border-purple-700 rounded-[3px] md:grid md:grid-cols-2  justify-center w-3/5 md:w-9/12 z-[99999] ' style={{ transform: 'translate(-50%, -50%)' }} >
        ...
        <div className="flex flex-col selection: bg-gradient-to-tl  from-purple-700 to-black-500 hover:shadow-soft-2xl relative border border-purple-700 p-[5px] ">
          {/* Top */}
